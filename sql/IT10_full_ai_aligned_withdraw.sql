...
-- =============================================
-- IT10 数据库（基于 IT9_v2 保持非 AI 模块不变，重构 AI 模块表）
-- 设计原则：
...
-- =============================================
-- =============================================
-- 16. AI 模块（按当前 it-module-ai 实体与编排逻辑重构）
-- 说明：
-- 1. 其余业务模块保持与 IT9_v2 一致，仅重构/新增 AI 相关数据表。
-- 2. conversation、message 等互动表不再承载 AI 字段，AI 会话独立使用 ai_session/ai_message。
-- 3. 由于当前 JPA 实体大量使用 @Enumerated(EnumType.STRING)，本版 AI 表统一采用 VARCHAR 存储枚举值，
--    以兼容实体中的大写枚举（如 ACTIVE、GENERAL_CHAT、PROJECT_ASSISTANT）。
-- 4. 本版数据库名为 it9_data。如程序仍连接 it9_data，请同步修改 datasource 配置。
-- =============================================
