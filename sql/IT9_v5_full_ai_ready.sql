...
-- 创建表：project_task_checklist_item
...
-- =============================================
-- 16. AI 模块重构表（保留并增强 ai_model，新增可直接支撑后续开发的 AI 数据结构）
-- 说明：
-- 1. conversation/message 保持为站内互动表，不再混入 AI 字段。
-- 2. AI 会话、AI 消息、知识库、日志、反馈全部使用独立表。
-- 3. 本版在 v4 的基础上补齐 10 条关键差距：业务绑定、会话知识库绑定、消息 Token 明细、
--    调用日志业务定位、反馈绑定消息、索引任务、切片向量表、知识库成员、会话摘要、模板发布状态与输出结构。
-- =============================================
...
CREATE TABLE `ai_model` (
  ...
  `model_name` varchar(100) NOT NULL COMMENT '模型名称，如 deepseek-chat、qwen-plus、llama3.1',
  `model_type` enum('openai','baidu','qwen','deepseek','ollama','custom') NOT NULL COMMENT '模型类型：openai、baidu、qwen、deepseek、ollama、custom',
  `provider_code` varchar(50) DEFAULT NULL COMMENT '模型提供方代码，便于程序按 provider 路由，例如 openai、deepseek、ollama',
  `deployment_mode` enum('cloud','local') NOT NULL DEFAULT 'cloud' COMMENT '部署模式：cloud 在线API，local 本地模型',
  `api_key` varchar(255) DEFAULT NULL COMMENT 'API密钥，本地模型可为空',
  `base_url` varchar(255) DEFAULT NULL COMMENT 'API基础地址或本地服务地址',
  `default_params` json DEFAULT NULL COMMENT '默认调用参数，JSON格式，如 temperature、max_tokens',
  `priority` int NOT NULL DEFAULT '100' COMMENT '模型优先级，数字越小优先级越高',
  `timeout_ms` int NOT NULL DEFAULT '30000' COMMENT '请求超时时间，单位毫秒',
  `supports_stream` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否支持流式输出：1支持，0不支持',
  `supports_tools` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否支持工具调用：1支持，0不支持',
  `supports_embedding` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否支持向量化：1支持，0不支持',
  `cost_input_per_1m` decimal(10,4) DEFAULT '0.0000' COMMENT '每100万输入Token成本，个人学习场景可填0',
  `cost_output_per_1m` decimal(10,4) DEFAULT '0.0000' COMMENT '每100万输出Token成本，个人学习场景可填0',
  `is_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用：1启用，0停用',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注，用于记录用途，如本地学习模型、线上备用模型',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ai_model_name` (`model_name`),
  KEY `idx_ai_model_provider_enabled` (`provider_code`,`is_enabled`,`priority`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI模型配置表：统一管理本地模型和在线模型的接入配置';

-- 创建表：ai_prompt_template
CREATE TABLE `ai_prompt_template` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '提示词模板ID，主键',
  `scene_code` varchar(100) NOT NULL COMMENT '场景编码，如 chat_general、project_summary、blog_rewrite、knowledge_qa',
  `name` varchar(100) NOT NULL COMMENT '模板名称',
  `template_type` enum('general_chat','knowledge_qa','project_assistant','summary','code_explain','rewrite','custom') NOT NULL DEFAULT 'custom' COMMENT '模板类型：general_chat普通聊天，knowledge_qa知识问答，project_assistant项目助手，summary摘要，code_explain代码解释，rewrite改写，custom自定义',
  `scope_type` enum('platform','project','personal') NOT NULL DEFAULT 'platform' COMMENT '模板作用域：platform 平台级，project 项目级，personal 个人级',
  `project_id` bigint DEFAULT NULL COMMENT '关联项目ID，当 scope_type=project 时使用，关联 project 表',
  `owner_id` bigint DEFAULT NULL COMMENT '模板创建者ID，关联 user_info 表',
  `default_model_id` bigint DEFAULT NULL COMMENT '默认模型ID，关联 ai_model 表',
  `system_prompt` longtext NOT NULL COMMENT '系统提示词，用于定义 AI 角色、规则和输出边界',
  `user_prompt_template` longtext COMMENT '用户提示词模板，支持 ${变量名} 形式占位',
  `variables_schema` json DEFAULT NULL COMMENT '模板变量定义，JSON格式，如变量名、说明、是否必填',
  `output_schema` json DEFAULT NULL COMMENT '输出结构约束，JSON格式，用于约束 AI 返回字段与格式',
  `publish_status` enum('draft','published','disabled') NOT NULL DEFAULT 'draft' COMMENT '模板发布状态：draft草稿，published已发布，disabled停用',
  `version_no` int NOT NULL DEFAULT '1' COMMENT '模板版本号，用于后续迭代管理',
  `is_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用：1启用，0停用',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注信息',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_prompt_scene` (`scene_code`),
  KEY `idx_ai_prompt_scope` (`scope_type`,`template_type`),
  KEY `idx_ai_prompt_project` (`project_id`),
  KEY `idx_ai_prompt_owner` (`owner_id`),
  KEY `idx_ai_prompt_model` (`default_model_id`),
  KEY `idx_ai_prompt_publish` (`publish_status`,`is_enabled`),
  CONSTRAINT `ai_prompt_template_ibfk_1` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE SET NULL,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI提示词模板表：统一管理不同业务场景下的系统提示词、输出结构和发布状态';
...
CREATE TABLE `knowledge_base` (
  ...
  `scope_type` enum('personal','project','platform') NOT NULL DEFAULT 'personal' COMMENT '作用域：personal个人知识库，project项目知识库，platform平台公共知识库',
  `project_id` bigint DEFAULT NULL COMMENT '关联项目ID，当作用域为项目知识库时使用，关联 project 表',
  `owner_id` bigint DEFAULT NULL COMMENT '知识库拥有者ID，关联 user_info 表',
  `source_type` enum('manual','upload','project_file','project_doc','blog','mixed') NOT NULL DEFAULT 'mixed' COMMENT '来源类型：manual手工录入，upload上传，project_file项目文件，project_doc项目文档，blog博客，mixed混合',
  `embedding_provider` varchar(50) DEFAULT NULL COMMENT '向量化提供方，如 ollama、openai、local',
  `embedding_model` varchar(100) DEFAULT NULL COMMENT '向量化模型名称',
  `chunk_strategy` enum('fixed','paragraph','markdown','custom') NOT NULL DEFAULT 'paragraph' COMMENT '切片策略：fixed固定长度，paragraph按段落，markdown按Markdown结构，custom自定义',
  `default_top_k` int NOT NULL DEFAULT '5' COMMENT '默认召回切片数量',
  `visibility` enum('private','team','public') NOT NULL DEFAULT 'private' COMMENT '可见性：private仅自己，team团队可见，public公开',
  `status` enum('draft','indexing','active','disabled','failed') NOT NULL DEFAULT 'draft' COMMENT '知识库状态：draft草稿，indexing索引中，active可用，disabled停用，failed失败',
  `doc_count` int NOT NULL DEFAULT '0' COMMENT '文档数量冗余统计',
  `chunk_count` int NOT NULL DEFAULT '0' COMMENT '切片数量冗余统计',
  `last_indexed_at` datetime DEFAULT NULL COMMENT '最近一次索引完成时间',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注信息',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_kb_scope` (`scope_type`,`project_id`),
  KEY `idx_kb_owner` (`owner_id`),
  KEY `idx_kb_status` (`status`),
  CONSTRAINT `knowledge_base_ibfk_1` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE SET NULL,
  CONSTRAINT `knowledge_base_ibfk_2` FOREIGN KEY (`owner_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识库主表：管理个人、项目或平台级知识库的基本信息';

-- 创建表：knowledge_base_member
CREATE TABLE `knowledge_base_member` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '知识库成员记录ID，主键',
  `knowledge_base_id` bigint NOT NULL COMMENT '知识库ID，关联 knowledge_base 表',
  `user_id` bigint NOT NULL COMMENT '成员用户ID，关联 user_info 表',
  `member_role` enum('owner','editor','viewer') NOT NULL DEFAULT 'viewer' COMMENT '成员角色：owner拥有者，editor可编辑，viewer可查看',
  `status` enum('active','removed') NOT NULL DEFAULT 'active' COMMENT '成员状态：active有效，removed已移除',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_kb_member_unique` (`knowledge_base_id`,`user_id`),
  KEY `idx_kb_member_user` (`user_id`,`status`),
  CONSTRAINT `knowledge_base_member_ibfk_1` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE CASCADE,
  CONSTRAINT `knowledge_base_member_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识库成员表：为项目知识库或团队知识库提供协作权限控制';

-- 创建表：knowledge_document
CREATE TABLE `knowledge_document` (
  ...
  `title` varchar(255) NOT NULL COMMENT '文档标题',
  `source_type` enum('upload','project_file','project_doc','blog','manual','url','other') NOT NULL DEFAULT 'manual' COMMENT '文档来源类型',
  `source_ref_id` bigint DEFAULT NULL COMMENT '来源业务记录ID，如 project_file.id、blog.id',
  `source_url` varchar(500) DEFAULT NULL COMMENT '外部来源URL',
  `file_name` varchar(255) DEFAULT NULL COMMENT '原始文件名',
  `mime_type` varchar(100) DEFAULT NULL COMMENT '文件MIME类型',
  `storage_path` varchar(500) DEFAULT NULL COMMENT '原始文件存储路径或对象存储地址',
  `content_text` longtext COMMENT '解析后的纯文本内容',
  `content_hash` varchar(64) DEFAULT NULL COMMENT '文档内容哈希值，用于去重和变更检测',
  `language` varchar(20) DEFAULT NULL COMMENT '文档语言，如 zh-CN、en-US',
  `status` enum('uploaded','parsing','indexed','failed','disabled') NOT NULL DEFAULT 'uploaded' COMMENT '文档处理状态',
  `error_message` varchar(500) DEFAULT NULL COMMENT '处理失败原因',
  `uploaded_by` bigint DEFAULT NULL COMMENT '上传人ID，关联 user_info 表',
  ...
  KEY `idx_kd_uploaded_by` (`uploaded_by`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识文档表：存储知识库中的原始文档和解析文本';

-- 创建表：knowledge_index_task
CREATE TABLE `knowledge_index_task` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '知识索引任务ID，主键',
  `knowledge_base_id` bigint NOT NULL COMMENT '知识库ID，关联 knowledge_base 表',
  `document_id` bigint DEFAULT NULL COMMENT '文档ID，关联 knowledge_document 表，可为空表示整库任务',
  `task_type` enum('parse','chunk','embed','reindex') NOT NULL COMMENT '任务类型：parse解析、chunk切片、embed向量化、reindex重建索引',
  `status` enum('pending','running','success','failed','cancelled') NOT NULL DEFAULT 'pending' COMMENT '任务状态：pending待执行，running执行中，success成功，failed失败，cancelled取消',
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_kit_kb_status` (`knowledge_base_id`,`status`),
  KEY `idx_kit_doc_type` (`document_id`,`task_type`),
  CONSTRAINT `knowledge_index_task_ibfk_1` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE CASCADE,
  CONSTRAINT `knowledge_index_task_ibfk_2` FOREIGN KEY (`document_id`) REFERENCES `knowledge_document` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识索引任务表：用于异步记录文档解析、切片、向量化和重建索引任务';
...
CREATE TABLE `knowledge_chunk` (
  ...
  `chunk_index` int NOT NULL COMMENT '在文档中的切片序号',
  `title` varchar(255) DEFAULT NULL COMMENT '切片标题',
  `content` longtext NOT NULL COMMENT '切片正文内容，用于召回和生成',
  `token_count` int DEFAULT NULL COMMENT '切片Token数量',
  `char_count` int DEFAULT NULL COMMENT '切片字符数',
  `embedding_provider` varchar(50) DEFAULT NULL COMMENT '向量化提供方',
  `embedding_model` varchar(100) DEFAULT NULL COMMENT '向量化模型名称',
  `vector_id` varchar(255) DEFAULT NULL COMMENT '外部向量ID，可为空，零成本方案可不使用',
  `metadata` json DEFAULT NULL COMMENT '附加元数据，JSON格式，如页码、标题层级、标签等',
  `status` enum('active','disabled') NOT NULL DEFAULT 'active' COMMENT '切片状态：active可用，disabled停用',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_kc_document_chunk` (`document_id`,`chunk_index`),
  KEY `idx_kc_kb` (`knowledge_base_id`),
  KEY `idx_kc_vector` (`vector_id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识切片表：存储知识库文档拆分后的片段及其检索元数据';

-- 创建表：knowledge_chunk_embedding
CREATE TABLE `knowledge_chunk_embedding` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '切片向量记录ID，主键',
  `chunk_id` bigint NOT NULL COMMENT '知识切片ID，关联 knowledge_chunk 表',
  `provider_code` varchar(50) NOT NULL COMMENT '向量提供方代码，如 local、ollama、openai',
  `model_name` varchar(100) NOT NULL COMMENT '向量模型名称',
  `dimension` int DEFAULT NULL COMMENT '向量维度',
  `vector_ref` varchar(255) DEFAULT NULL COMMENT '外部向量库引用ID',
  `vector_payload` longtext COMMENT '向量内容快照，零成本本地方案可存 JSON 字符串',
  `status` enum('active','stale','disabled') NOT NULL DEFAULT 'active' COMMENT '向量状态：active可用，stale过期待重建，disabled停用',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_kce_chunk_model` (`chunk_id`,`provider_code`,`model_name`),
  KEY `idx_kce_model_status` (`model_name`,`status`),
  CONSTRAINT `knowledge_chunk_embedding_ibfk_1` FOREIGN KEY (`chunk_id`) REFERENCES `knowledge_chunk` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识切片向量表：支持一个切片对应多个向量版本，适配零成本本地向量和外部向量库';

-- 创建表：ai_session
CREATE TABLE `ai_session` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'AI会话ID，主键',
  `user_id` bigint NOT NULL COMMENT '发起会话的用户ID，关联 user_info 表',
  `project_id` bigint DEFAULT NULL COMMENT '兼容保留的项目ID，项目类场景时可直接使用，关联 project 表',
  `biz_type` enum('general','project','blog','circle','paid_content','other') NOT NULL DEFAULT 'general' COMMENT '业务类型：general通用，project项目，blog博客，circle圈子，paid_content知识付费内容，other其他',
  `biz_id` bigint DEFAULT NULL COMMENT '业务主记录ID，结合 biz_type 使用，实现多业务统一绑定',
  `scene_code` varchar(100) NOT NULL COMMENT '场景编码，如 chat_general、project_summary、blog_rewrite、knowledge_qa',
  `title` varchar(200) DEFAULT NULL COMMENT '会话标题',
  `status` enum('active','archived','closed') NOT NULL DEFAULT 'active' COMMENT '会话状态：active进行中，archived归档，closed关闭',
  `memory_mode` enum('none','short','long') NOT NULL DEFAULT 'short' COMMENT '记忆模式：none无记忆，short短期上下文，long长期上下文',
  `session_summary` longtext COMMENT '会话摘要，用于长会话压缩记忆',
  `summary_updated_at` datetime DEFAULT NULL COMMENT '会话摘要最近更新时间',
  ...
  `knowledge_base_ids` json DEFAULT NULL COMMENT '兼容保留的知识库ID列表，JSON数组形式存储，新逻辑推荐使用 ai_session_knowledge_base 表',
  `ext_config` json DEFAULT NULL COMMENT '扩展配置，JSON格式，如温度、top_p、是否启用引用',
  `last_message_at` datetime DEFAULT NULL COMMENT '最后一条消息时间',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_session_user_scene` (`user_id`,`scene_code`),
  KEY `idx_ai_session_project` (`project_id`),
  KEY `idx_ai_session_biz` (`biz_type`,`biz_id`),
  KEY `idx_ai_session_model` (`active_model_id`),
  KEY `idx_ai_session_prompt` (`prompt_template_id`),
  KEY `idx_ai_session_kb` (`default_knowledge_base_id`),
  CONSTRAINT `ai_session_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE,
  CONSTRAINT `ai_session_ibfk_2` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_session_ibfk_3` FOREIGN KEY (`active_model_id`) REFERENCES `ai_model` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_session_ibfk_4` FOREIGN KEY (`prompt_template_id`) REFERENCES `ai_prompt_template` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_session_ibfk_5` FOREIGN KEY (`default_knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI专用会话表：与通用站内私信会话彻底分离，支持多业务绑定、会话摘要和默认配置';

-- 创建表：ai_session_knowledge_base
CREATE TABLE `ai_session_knowledge_base` (
  ...
  `knowledge_base_id` bigint NOT NULL COMMENT '知识库ID，关联 knowledge_base 表',
  `priority` int NOT NULL DEFAULT '100' COMMENT '知识库优先级，数字越小越优先',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ai_session_kb_unique` (`session_id`,`knowledge_base_id`),
  KEY `idx_ai_session_kb_priority` (`session_id`,`priority`),
  CONSTRAINT `ai_session_knowledge_base_ibfk_1` FOREIGN KEY (`session_id`) REFERENCES `ai_session` (`id`) ON DELETE CASCADE,
  CONSTRAINT `ai_session_knowledge_base_ibfk_2` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI会话知识库绑定表：替代 ai_session 中的 JSON 绑定方式，便于检索、权限和统计';

-- 创建表：ai_message
CREATE TABLE `ai_message` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'AI消息ID，主键',
  `session_id` bigint NOT NULL COMMENT '所属AI会话ID，关联 ai_session 表',
  `role` enum('system','user','assistant','tool') NOT NULL COMMENT '消息角色：system系统，user用户，assistant助手，tool工具',
  `sender_user_id` bigint DEFAULT NULL COMMENT '发送用户ID，当 role=user 时通常有值，关联 user_info 表',
  `content` longtext NOT NULL COMMENT '消息正文',
  `content_tokens` int DEFAULT NULL COMMENT '消息内容自身Token数量，保留用于快速估算',
  `prompt_tokens` int DEFAULT NULL COMMENT '本条生成使用的输入Token数量',
  `completion_tokens` int DEFAULT NULL COMMENT '本条生成产生的输出Token数量',
  `total_tokens` int DEFAULT NULL COMMENT '本条生成总Token数量',
  `prompt_template_id` bigint DEFAULT NULL COMMENT '本条消息实际使用的提示词模板ID，关联 ai_prompt_template 表',
  `model_id` bigint DEFAULT NULL COMMENT '本条消息实际使用的模型ID，关联 ai_model 表',
  `knowledge_base_id` bigint DEFAULT NULL COMMENT '本条消息默认关联的知识库ID，关联 knowledge_base 表',
  `quoted_chunk_ids` json DEFAULT NULL COMMENT '本次回答引用的知识切片ID列表，JSON数组形式存储',
  `tool_call_json` json DEFAULT NULL COMMENT '工具调用记录，JSON格式存储',
  `latency_ms` int DEFAULT NULL COMMENT '生成耗时，单位毫秒',
  `finish_reason` varchar(50) DEFAULT NULL COMMENT '生成结束原因，如 stop、length、error',
  `status` enum('success','failed','streaming') NOT NULL DEFAULT 'success' COMMENT '消息处理状态',
  `error_message` varchar(500) DEFAULT NULL COMMENT '失败原因',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_message_session` (`session_id`,`created_at`),
  KEY `idx_ai_message_sender` (`sender_user_id`),
  KEY `idx_ai_message_model` (`model_id`),
  KEY `idx_ai_message_prompt` (`prompt_template_id`),
  CONSTRAINT `ai_message_ibfk_1` FOREIGN KEY (`session_id`) REFERENCES `ai_session` (`id`) ON DELETE CASCADE,
  CONSTRAINT `ai_message_ibfk_2` FOREIGN KEY (`sender_user_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_message_ibfk_3` FOREIGN KEY (`prompt_template_id`) REFERENCES `ai_prompt_template` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_message_ibfk_4` FOREIGN KEY (`model_id`) REFERENCES `ai_model` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_message_ibfk_5` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI消息表：记录 AI 会话中的系统、用户、助手及工具消息，并保存输入输出 Token 细节';

-- 创建表：ai_call_log
CREATE TABLE `ai_call_log` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'AI调用日志ID，主键',
  `user_id` bigint DEFAULT NULL COMMENT '发起调用的用户ID，关联 user_info 表',
  ...
  `prompt_template_id` bigint DEFAULT NULL COMMENT '使用的提示词模板ID，关联 ai_prompt_template 表',
  `ai_model_id` bigint DEFAULT NULL COMMENT '实际使用的模型ID，关联 ai_model 表',
  `knowledge_base_id` bigint DEFAULT NULL COMMENT '主要使用的知识库ID，关联 knowledge_base 表',
  `request_type` enum('chat','knowledge_qa','summary','rewrite','project_assistant','code_explain','other') NOT NULL DEFAULT 'chat' COMMENT '调用类型',
  `request_text` longtext COMMENT '请求原文',
  `response_text` longtext COMMENT '响应原文，可按需要裁剪存储',
  `request_params` json DEFAULT NULL COMMENT '调用参数，JSON格式',
  ...
  `cost_amount` decimal(10,4) DEFAULT '0.0000' COMMENT '本次调用成本金额，零成本学习场景通常记录为0',
  `latency_ms` int DEFAULT NULL COMMENT '调用耗时，单位毫秒',
  `status` enum('success','failed','timeout','cancelled') NOT NULL DEFAULT 'success' COMMENT '调用状态',
  `error_code` varchar(100) DEFAULT NULL COMMENT '错误码',
  `error_message` varchar(500) DEFAULT NULL COMMENT '错误信息',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_call_user` (`user_id`),
  KEY `idx_ai_call_project` (`project_id`),
  KEY `idx_ai_call_biz` (`biz_type`,`biz_id`,`created_at`),
  KEY `idx_ai_call_session` (`session_id`),
  KEY `idx_ai_call_model` (`ai_model_id`),
  KEY `idx_ai_call_type_created` (`request_type`,`created_at`),
  ...
  CONSTRAINT `ai_call_log_ibfk_3` FOREIGN KEY (`session_id`) REFERENCES `ai_session` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_call_log_ibfk_4` FOREIGN KEY (`message_id`) REFERENCES `ai_message` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_call_log_ibfk_5` FOREIGN KEY (`prompt_template_id`) REFERENCES `ai_prompt_template` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_call_log_ibfk_6` FOREIGN KEY (`ai_model_id`) REFERENCES `ai_model` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_call_log_ibfk_7` FOREIGN KEY (`knowledge_base_id`) REFERENCES `knowledge_base` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI调用日志表：记录请求、响应、Token、耗时、成本及业务归属';

-- 创建表：ai_retrieval_log
CREATE TABLE `ai_retrieval_log` (
  ...
  `knowledge_base_id` bigint DEFAULT NULL COMMENT '命中的知识库ID，关联 knowledge_base 表',
  `document_id` bigint DEFAULT NULL COMMENT '命中的知识文档ID，关联 knowledge_document 表',
  `chunk_id` bigint DEFAULT NULL COMMENT '命中的知识切片ID，关联 knowledge_chunk 表',
  `query_text` text COMMENT '检索查询文本',
  `score` decimal(10,6) DEFAULT NULL COMMENT '召回得分或排序分值',
  `rank_no` int DEFAULT NULL COMMENT '排序名次',
  `retrieval_method` enum('vector','keyword','hybrid','manual') NOT NULL DEFAULT 'vector' COMMENT '检索方式',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_retrieval_call` (`call_log_id`),
  KEY `idx_ai_retrieval_kb` (`knowledge_base_id`),
  KEY `idx_ai_retrieval_doc` (`document_id`),
  KEY `idx_ai_retrieval_chunk` (`chunk_id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI检索日志表：记录一次问答命中了哪些知识库、文档与切片';

-- 创建表：ai_feedback_log
CREATE TABLE `ai_feedback_log` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'AI反馈日志ID，主键',
  `call_log_id` bigint NOT NULL COMMENT '所属AI调用日志ID，关联 ai_call_log 表',
  `message_id` bigint DEFAULT NULL COMMENT '对应的AI消息ID，关联 ai_message 表',
  `user_id` bigint DEFAULT NULL COMMENT '提交反馈的用户ID，关联 user_info 表',
  `feedback_type` enum('like','dislike','accepted','retry','report') NOT NULL COMMENT '反馈类型：like点赞，dislike点踩，accepted已采纳，retry重新生成，report问题反馈',
  `comment_text` varchar(500) DEFAULT NULL COMMENT '用户反馈说明',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_ai_feedback_call` (`call_log_id`),
  KEY `idx_ai_feedback_message` (`message_id`),
  KEY `idx_ai_feedback_user` (`user_id`),
  CONSTRAINT `ai_feedback_log_ibfk_1` FOREIGN KEY (`call_log_id`) REFERENCES `ai_call_log` (`id`) ON DELETE CASCADE,
  CONSTRAINT `ai_feedback_log_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `ai_feedback_log_ibfk_3` FOREIGN KEY (`message_id`) REFERENCES `ai_message` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='AI反馈日志表：记录用户对AI回答的评价、采纳情况以及对应消息';

-- =============================================
-- 17. 创作者提现表（基于知识付费收益模块扩展）
-- 说明：
-- 1. 提现只面向作者收益 author_revenue，不改变原有订单、支付、收益表逻辑。
-- 2. 通过 creator_withdraw_item 与 revenue_record 关联，避免同一收益重复提现。
-- =============================================
...
-- 创建表：creator_settlement_account
CREATE TABLE `creator_settlement_account` (
  ...
  `account_type` enum('alipay','wechat','bank_card','manual') NOT NULL COMMENT '账户类型：alipay支付宝，wechat微信，bank_card银行卡，manual线下人工',
  ...
  `status` enum('active','disabled') NOT NULL DEFAULT 'active' COMMENT '账户状态：active启用，disabled停用',
  `remark` varchar(255) DEFAULT NULL COMMENT '备注信息',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_creator_account_user` (`user_id`,`status`),
  CONSTRAINT `creator_settlement_account_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='创作者结算账户表：管理作者提现时可使用的收款账户';

-- 创建表：creator_withdraw_request
CREATE TABLE `creator_withdraw_request` (
  ...
  `status` enum('pending','approved','rejected','paid','cancelled') NOT NULL DEFAULT 'pending' COMMENT '申请状态：pending待审核，approved已通过待打款，rejected已拒绝，paid已打款，cancelled已取消',
  ...
  CONSTRAINT `creator_withdraw_request_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE,
  CONSTRAINT `creator_withdraw_request_ibfk_2` FOREIGN KEY (`settlement_account_id`) REFERENCES `creator_settlement_account` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `creator_withdraw_request_ibfk_3` FOREIGN KEY (`reviewed_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='创作者提现申请表：记录提现金额、审核状态和打款信息';

-- 创建表：creator_withdraw_item
CREATE TABLE `creator_withdraw_item` (
  ...
  CONSTRAINT `creator_withdraw_item_ibfk_1` FOREIGN KEY (`withdraw_request_id`) REFERENCES `creator_withdraw_request` (`id`) ON DELETE CASCADE,
  CONSTRAINT `creator_withdraw_item_ibfk_2` FOREIGN KEY (`revenue_record_id`) REFERENCES `revenue_record` (`id`) ON DELETE RESTRICT
