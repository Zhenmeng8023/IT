...
CREATE TABLE `ai_call_log` (
  ...
  `request_id` varchar(64) DEFAULT NULL COMMENT 'External or generated request id.',
  `trace_id` varchar(64) DEFAULT NULL COMMENT 'Trace id across retrieval/model/streaming.',
  `repository_id` bigint DEFAULT NULL COMMENT 'Related project_repository for code calls.',
  `branch_id` bigint DEFAULT NULL COMMENT 'Related project_branch for code calls.',
  `commit_id` bigint DEFAULT NULL COMMENT 'Related project_commit snapshot for code calls.',
  `request_stage` varchar(30) DEFAULT NULL COMMENT 'PLAN, RETRIEVE, RERANK, GROUND, GENERATE, STREAM.',
  `retrieval_plan_json` json DEFAULT NULL COMMENT 'Retrieval plan including declaration-first and graph expansion.',
  `retrieval_summary_json` json DEFAULT NULL COMMENT 'Retrieval summary and selected evidence.',
  `grounding_report_json` json DEFAULT NULL COMMENT 'Strict grounding result.',
  `degrade_reason` varchar(500) DEFAULT NULL COMMENT 'Reason for degraded retrieval/generation.',
  `stream_started_at` timestamp NULL DEFAULT NULL COMMENT 'Streaming start time.',
  `stream_finished_at` timestamp NULL DEFAULT NULL COMMENT 'Streaming finish time.',
  `first_token_latency_ms` int DEFAULT NULL COMMENT 'First-token latency in ms.',
  `metadata_json` json DEFAULT NULL COMMENT 'Provider/runtime metadata.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_ai_call_log_trace` (`trace_id`,`created_at`),
  KEY `idx_ai_call_log_request_id` (`request_id`),
  KEY `idx_ai_call_log_stage_status` (`request_stage`,`status`,`created_at`),
  KEY `idx_ai_call_log_repo` (`repository_id`,`branch_id`,`commit_id`),
  KEY `fk_ai_call_log_branch` (`branch_id`),
  KEY `fk_ai_call_log_commit` (`commit_id`),
  CONSTRAINT `fk_ai_call_log_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_call_log_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  ...
  CONSTRAINT `fk_ai_call_log_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
--
-- Table structure for table `ai_code_reference`
--

DROP TABLE IF EXISTS `ai_code_reference`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ai_code_reference` (
  ...
  KEY `fk_ai_code_reference_repository` (`repository_id`),
  KEY `fk_ai_code_reference_branch` (`branch_id`),
  KEY `fk_ai_code_reference_commit` (`commit_id`),
  KEY `fk_ai_code_reference_from_chunk` (`from_chunk_id`),
  KEY `fk_ai_code_reference_to_doc` (`to_document_id`),
  KEY `fk_ai_code_reference_to_chunk` (`to_chunk_id`),
...
--
-- Table structure for table `ai_code_symbol`
--

DROP TABLE IF EXISTS `ai_code_symbol`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ai_code_symbol` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `knowledge_base_id` bigint NOT NULL COMMENT 'Owning knowledge base.',
  ...
  KEY `fk_ai_code_symbol_repository` (`repository_id`),
  KEY `fk_ai_code_symbol_branch` (`branch_id`),
  KEY `fk_ai_code_symbol_commit` (`commit_id`),
  KEY `fk_ai_code_symbol_project_file` (`project_file_id`),
  KEY `fk_ai_code_symbol_project_file_version` (`project_file_version_id`),
...
) ENGINE=InnoDB07 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Code symbols for declaration-first retrieval and graph expansion.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `ai_message` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `parent_message_id` bigint DEFAULT NULL COMMENT 'Parent assistant message for streamed partial rows.',
  `turn_index` int DEFAULT NULL COMMENT 'Conversation turn index.',
  `content_format` varchar(30) NOT NULL DEFAULT 'TEXT' COMMENT 'TEXT, MARKDOWN, JSON, TOOL_RESULT.',
  `stream_state` varchar(20) NOT NULL DEFAULT 'FINAL' COMMENT 'FINAL, PARTIAL, DELTA, CANCELLED.',
  `partial_seq` int NOT NULL DEFAULT '0' COMMENT 'Streaming partial sequence.',
  `delta_content` mediumtext COMMENT 'Delta payload for streamed partial answer.',
  `retrieval_summary_json` json DEFAULT NULL COMMENT 'Retrieval summary used for this message.',
  `grounding_status` varchar(30) DEFAULT NULL COMMENT 'STRICT_PASS, STRICT_FAIL, PARTIAL, NOT_CHECKED.',
  `grounding_json` json DEFAULT NULL COMMENT 'Grounding evidence and violations.',
  `citation_json` json DEFAULT NULL COMMENT 'Structured citations to chunks/symbols/references.',
  `updated_at` timestamp NULL DEFAULT NULL COMMENT 'Message update time for streaming.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_ai_message_parent_stream` (`parent_message_id`,`stream_state`,`partial_seq`),
  KEY `idx_ai_message_session_turn` (`session_id`,`turn_index`,`created_at`),
  KEY `idx_ai_message_grounding` (`grounding_status`,`created_at`),
  ...
  CONSTRAINT `fk_ai_message_parent` FOREIGN KEY (`parent_message_id`) REFERENCES `ai_message` (`id`) ON DELETE SET NULL,
...
CREATE TABLE `ai_retrieval_log` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `session_id` bigint DEFAULT NULL COMMENT 'Backfilled from call log for faster lookup.',
  `message_id` bigint DEFAULT NULL COMMENT 'Backfilled from call log for faster lookup.',
  `stage_code` varchar(30) NOT NULL DEFAULT 'RECALL' COMMENT 'PLAN, RECALL, DECLARATION_FIRST, GRAPH_EXPAND, RERANK, GROUND.',
  `stage_order` int DEFAULT NULL COMMENT 'Order within a retrieval pipeline.',
  `phase` varchar(50) DEFAULT NULL COMMENT 'Retrieval pipeline phase label.',
  `query_variant` text COMMENT 'Expanded or rewritten query variant.',
  `plan_step_json` json DEFAULT NULL COMMENT 'Retrieval plan step details.',
  `candidate_source` varchar(30) DEFAULT NULL COMMENT 'CHUNK, DOCUMENT, SYMBOL, REFERENCE.',
  `symbol_id` bigint DEFAULT NULL COMMENT 'Candidate code symbol.',
  `reference_id` bigint DEFAULT NULL COMMENT 'Candidate code reference edge.',
  `score_vector` decimal(10,6) DEFAULT NULL COMMENT 'Vector recall score.',
  `score_keyword` decimal(10,6) DEFAULT NULL COMMENT 'Keyword/BM25 score.',
  `score_graph` decimal(10,6) DEFAULT NULL COMMENT 'Graph expansion score.',
  `score_rerank` decimal(10,6) DEFAULT NULL COMMENT 'Rerank score.',
  `rerank_model` varchar(100) DEFAULT NULL COMMENT 'Rerank model name.',
  `grounding_status` varchar(30) DEFAULT NULL COMMENT 'Evidence grounding status.',
  `grounding_evidence_json` json DEFAULT NULL COMMENT 'Grounding evidence payload.',
  `degrade_reason` varchar(500) DEFAULT NULL COMMENT 'Retrieval degradation reason.',
  `metadata_json` json DEFAULT NULL COMMENT 'Retriever metadata.',
  `hit_reason_json` json DEFAULT NULL COMMENT 'Hit reason payload.',
  `score_detail_json` json DEFAULT NULL COMMENT 'Score detail payload.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_ai_retrieval_stage_rank` (`call_log_id`,`stage_code`,`stage_order`,`rank_no`),
  KEY `idx_ai_retrieval_session_message` (`session_id`,`message_id`,`created_at`),
  KEY `idx_ai_retrieval_symbol` (`symbol_id`,`stage_code`,`rank_no`),
  KEY `idx_ai_retrieval_reference` (`reference_id`,`stage_code`,`rank_no`),
  KEY `idx_ai_retrieval_candidate` (`candidate_source`,`retrieval_method`,`score_rerank`),
  KEY `fk_ai_retrieval_log_message` (`message_id`),
  ...
  CONSTRAINT `fk_ai_retrieval_log_message` FOREIGN KEY (`message_id`) REFERENCES `ai_message` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_retrieval_log_reference` FOREIGN KEY (`reference_id`) REFERENCES `ai_code_reference` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_retrieval_log_session` FOREIGN KEY (`session_id`) REFERENCES `ai_session` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_retrieval_log_symbol` FOREIGN KEY (`symbol_id`) REFERENCES `ai_code_symbol` (`id`) ON DELETE SET NULL
...
CREATE TABLE `ai_session` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `analysis_profile` varchar(50) DEFAULT NULL COMMENT 'AI analysis profile, e.g. CODE_ANALYSIS.',
  `repository_id` bigint DEFAULT NULL COMMENT 'Related project_repository for code sessions.',
  `branch_id` bigint DEFAULT NULL COMMENT 'Related project_branch for code sessions.',
  `commit_id` bigint DEFAULT NULL COMMENT 'Related project_commit snapshot for code sessions.',
  `retrieval_policy_json` json DEFAULT NULL COMMENT 'Per-session retrieval policy.',
  `grounding_policy_json` json DEFAULT NULL COMMENT 'Per-session grounding policy.',
  `context_snapshot_json` json DEFAULT NULL COMMENT 'Pinned code/context snapshot.',
  `last_retrieval_summary_json` json DEFAULT NULL COMMENT 'Last retrieval summary for UI/debug.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_ai_session_project_scene` (`project_id`,`scene_code`,`status`,`updated_at`),
  KEY `idx_ai_session_repo_branch` (`repository_id`,`branch_id`,`commit_id`),
  KEY `fk_ai_session_branch` (`branch_id`),
  KEY `fk_ai_session_commit` (`commit_id`),
  CONSTRAINT `fk_ai_session_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_session_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  ...
  CONSTRAINT `fk_ai_session_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
--
-- Table structure for table `ai_user_knowledge_usage_action_log`
--

DROP TABLE IF EXISTS `ai_user_knowledge_usage_action_log`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ai_user_knowledge_usage_action_log` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `user_id` bigint NOT NULL COMMENT '被治理用户ID',
  `action_code` varchar(50) NOT NULL COMMENT '治理动作编码',
  `reason` varchar(255) DEFAULT NULL COMMENT '操作原因',
  `operator_id` bigint DEFAULT NULL COMMENT '操作人ID',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_ai_user_knowledge_usage_action_user` (`user_id`),
  KEY `idx_ai_user_knowledge_usage_action_operator` (`operator_id`),
  CONSTRAINT `fk_ai_user_knowledge_usage_action_operator` FOREIGN KEY (`operator_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_user_knowledge_usage_action_user` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户知识库治理动作日志表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `ai_user_knowledge_usage_policy`
--

DROP TABLE IF EXISTS `ai_user_knowledge_usage_policy`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ai_user_knowledge_usage_policy` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `user_id` bigint NOT NULL COMMENT '用户ID，关联 user_info.id',
  `frozen` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否冻结知识库能力',
  `import_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否允许知识库导入',
  `qa_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否允许知识库问答',
  `max_knowledge_base_count` int NOT NULL DEFAULT '20' COMMENT '最大知识库数量',
  `max_document_count` int NOT NULL DEFAULT '500' COMMENT '最大文档数量',
  `max_daily_question_count` int NOT NULL DEFAULT '200' COMMENT '每日最大问答次数',
  `max_daily_import_count` int NOT NULL DEFAULT '50' COMMENT '每日最大导入次数',
  `remark` varchar(255) DEFAULT NULL COMMENT '配额或治理备注',
  `updated_by` bigint DEFAULT NULL COMMENT '最后操作人',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ai_user_knowledge_usage_policy_user` (`user_id`),
  KEY `idx_ai_user_knowledge_usage_policy_updated_by` (`updated_by`),
  CONSTRAINT `fk_ai_user_knowledge_usage_policy_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_ai_user_knowledge_usage_policy_user` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户知识库能力与配额配置表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0133 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='博客内容表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `circle` (
  ...
  `type` enum('official','private','public','pending','approved','rejected','close') NOT NULL DEFAULT 'pending' COMMENT '圈子状态(兼容旧值)',
...
) ENGINE=InnoDB0014 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='兴趣圈子表，支持群聊功能';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `circle_comment` (
  ...
  `post_id` bigint DEFAULT NULL COMMENT '主题帖首次插入可为空，二次更新为自身ID',
  ...
  `status` enum('pending','published','deleted','normal','hidden','approved','close','closed') DEFAULT 'pending',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB0383 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='圈子评论表，用于存储圈子内的主题帖和对主题的回帖';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0301 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='圈子成员关系及权限表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0229 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='评论表，支持对博客和项目进行评论和回复';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0225 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户关注关系表（单向）';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB053 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户好友关系表（双向确认）';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `knowledge_chunk` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `chunk_type` varchar(30) NOT NULL DEFAULT 'TEXT' COMMENT 'TEXT, CODE_BLOCK, SYMBOL_DECL, SYMBOL_BODY.',
  `file_path` varchar(1000) DEFAULT NULL COMMENT 'Source file path inherited from document.',
  `file_path_hash` char(64) GENERATED ALWAYS AS ((case when (`file_path` is null) then NULL else upper(sha2(`file_path`,256)) end)) STORED COMMENT 'SHA-256 hash of file_path.',
  `start_line` int DEFAULT NULL COMMENT 'Chunk start line for code/document grounding.',
  `start_column` int DEFAULT NULL COMMENT 'Chunk start column.',
  `end_line` int DEFAULT NULL COMMENT 'Chunk end line for code/document grounding.',
  `end_column` int DEFAULT NULL COMMENT 'Chunk end column.',
  `section_path` varchar(500) DEFAULT NULL COMMENT 'Heading or code scope path.',
  `primary_symbol_id` bigint DEFAULT NULL COMMENT 'Primary code symbol represented by this chunk.',
  `content_hash` char(64) DEFAULT NULL COMMENT 'SHA-256 hash of chunk content.',
  `rank_boost` decimal(8,4) DEFAULT NULL COMMENT 'Manual or parser-provided rank boost.',
  `retrieval_tags` json DEFAULT NULL COMMENT 'Tags used by retrieval filters.',
  `updated_at` timestamp NULL DEFAULT NULL COMMENT 'Chunk update time.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_kc_doc_line` (`document_id`,`start_line`,`end_line`),
  KEY `idx_kc_file_line` (`knowledge_base_id`,`file_path_hash`,`start_line`),
  KEY `idx_kc_chunk_type` (`knowledge_base_id`,`chunk_type`,`chunk_index`),
  KEY `idx_kc_primary_symbol` (`primary_symbol_id`),
  KEY `idx_kc_content_hash` (`content_hash`),
  CONSTRAINT `fk_kc_primary_symbol` FOREIGN KEY (`primary_symbol_id`) REFERENCES `ai_code_symbol` (`id`) ON DELETE SET NULL,
...
) ENGINE=InnoDB0 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识切片向量表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `knowledge_document` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `project_id` bigint DEFAULT NULL COMMENT 'Related project for code documents.',
  `repository_id` bigint DEFAULT NULL COMMENT 'Related project_repository.',
  `branch_id` bigint DEFAULT NULL COMMENT 'Related project_branch.',
  `commit_id` bigint DEFAULT NULL COMMENT 'Related project_commit.',
  `project_file_id` bigint DEFAULT NULL COMMENT 'Related project_file.',
  `project_file_version_id` bigint DEFAULT NULL COMMENT 'Related project_file_version.',
  `doc_kind` varchar(30) NOT NULL DEFAULT 'DOCUMENT' COMMENT 'DOCUMENT, CODE_FILE, README, API_DOC.',
  `file_path` varchar(1000) DEFAULT NULL COMMENT 'Canonical source path for code/document retrieval.',
  `file_path_hash` char(64) GENERATED ALWAYS AS ((case when (`file_path` is null) then NULL else upper(sha2(`file_path`,256)) end)) STORED COMMENT 'SHA-256 hash of file_path.',
  `parser_name` varchar(100) DEFAULT NULL COMMENT 'Parser used for code/document extraction.',
  `parser_version` varchar(50) DEFAULT NULL COMMENT 'Parser version.',
  `parse_status` varchar(30) NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING, PARSED, FAILED, NOT_APPLICABLE.',
  `symbol_index_status` varchar(30) NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING, INDEXED, FAILED, NOT_APPLICABLE.',
  `symbol_count` int NOT NULL DEFAULT '0' COMMENT 'Indexed symbol count.',
  `reference_count` int NOT NULL DEFAULT '0' COMMENT 'Indexed reference count.',
  `metadata_json` json DEFAULT NULL COMMENT 'Document parser metadata.',
  `source_updated_at` timestamp NULL DEFAULT NULL COMMENT 'Source file update time.',
  PRIMARY KEY (`id`),
  ...
  KEY `idx_kd_code_scope` (`knowledge_base_id`,`doc_kind`,`status`,`indexed_at`),
  KEY `idx_kd_repo_file_hash` (`repository_id`,`file_path_hash`,`commit_id`),
  KEY `idx_kd_project_file_version` (`project_id`,`project_file_id`,`project_file_version_id`),
  KEY `idx_kd_symbol_status` (`symbol_index_status`,`parse_status`,`updated_at`),
  KEY `fk_kd_branch` (`branch_id`),
  KEY `fk_kd_commit` (`commit_id`),
  KEY `fk_kd_project_file` (`project_file_id`),
  KEY `fk_kd_project_file_version` (`project_file_version_id`),
  CONSTRAINT `fk_kd_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_kd_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_kd_project` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_kd_project_file` FOREIGN KEY (`project_file_id`) REFERENCES `project_file` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_kd_project_file_version` FOREIGN KEY (`project_file_version_id`) REFERENCES `project_file_version` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_kd_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
) ENGINE=InnoDB01 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='知识库文档表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB00 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='前端菜单及权限配置表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `notification` (
  ...
  `category` varchar(50) NOT NULL DEFAULT 'system' COMMENT 'High-level category: system, interaction, project, invite, request, message, order, payment, ai',
  `type` varchar(50) NOT NULL COMMENT 'Concrete notification type',
  `title` varchar(120) NOT NULL DEFAULT '' COMMENT 'Notification title',
  `content` text NOT NULL COMMENT 'Notification content',
  `read_status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '0 unread, 1 read',
  `read_at` timestamp NULL DEFAULT NULL COMMENT 'Read time',
  `target_type` varchar(80) DEFAULT NULL COMMENT 'Target business type',
  `target_id` bigint DEFAULT NULL COMMENT '被操作的目标ID',
  `source_type` varchar(80) DEFAULT NULL COMMENT 'Source event/business type for idempotency and tracing',
  `source_id` bigint DEFAULT NULL COMMENT 'Source event/business id',
  `event_key` varchar(191) DEFAULT NULL COMMENT 'Idempotency key, for example project_invitation:123:created:user:8',
  `action_url` varchar(500) DEFAULT NULL COMMENT 'Frontend route opened when clicking the notification',
  `business_status` varchar(30) NOT NULL DEFAULT 'open' COMMENT 'open, handled, cancelled, expired, failed',
  `priority` int NOT NULL DEFAULT '0' COMMENT 'Larger means more important',
  `payload_json` json DEFAULT NULL COMMENT 'Extensible structured payload for UI actions and future modules',
  ...
  KEY `idx_notification_receiver_type_created` (`receiver_id`,`type`,`created_at` DESC),
  ...
  KEY `idx_notification_deleted_created` (`deleted_at`,`created_at` DESC),
...
CREATE TABLE `notification_delivery` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'Delivery id',
  `notification_id` bigint NOT NULL COMMENT 'Notification id',
  `channel` varchar(30) NOT NULL DEFAULT 'in_app' COMMENT 'in_app, sse, email, sms, push',
  `status` varchar(30) NOT NULL DEFAULT 'pending' COMMENT 'pending, sent, failed, skipped',
  `retry_count` int NOT NULL DEFAULT '0' COMMENT 'Retry count',
  `last_error` varchar(500) DEFAULT NULL COMMENT 'Last delivery error',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Notification delivery status for future channels';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `notification_event_log`
--

DROP TABLE IF EXISTS `notification_event_log`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `notification_event_log` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'Event log id',
  ...
  `source_type` varchar(80) DEFAULT NULL COMMENT 'Source business type',
  `source_id` bigint DEFAULT NULL COMMENT 'Source business id',
  `target_type` varchar(80) DEFAULT NULL COMMENT 'Target business type',
  `target_id` bigint DEFAULT NULL COMMENT 'Target business id',
  `receiver_count` int NOT NULL DEFAULT '0' COMMENT 'Number of receivers generated by this event',
  `status` varchar(30) NOT NULL DEFAULT 'created' COMMENT 'created, dispatched, skipped, failed',
  `error_message` varchar(500) DEFAULT NULL COMMENT 'Error message if dispatch failed',
  `payload_json` json DEFAULT NULL COMMENT 'Raw event payload',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_notification_event_log_key` (`event_key`),
  KEY `idx_notification_event_log_source` (`source_type`,`source_id`),
  KEY `idx_notification_event_log_status_created` (`status`,`created_at` DESC),
  KEY `fk_notification_event_log_sender` (`sender_id`),
  CONSTRAINT `fk_notification_event_log_sender` FOREIGN KEY (`sender_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Notification event idempotency and dispatch log';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `notification_preference` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'Preference id',
  `user_id` bigint NOT NULL COMMENT 'User id',
  `category` varchar(50) NOT NULL DEFAULT '*' COMMENT 'Category or * for all categories',
  `type` varchar(50) NOT NULL DEFAULT '*' COMMENT 'Type or * for all types',
  `in_app_enabled` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'In-app notification enabled',
  `email_enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Email notification enabled',
  `sms_enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'SMS notification enabled',
  `push_enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Push notification enabled',
  `quiet_start` time DEFAULT NULL COMMENT 'Quiet hours start',
  `quiet_end` time DEFAULT NULL COMMENT 'Quiet hours end',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_notification_preference_user_scope` (`user_id`,`category`,`type`),
  KEY `idx_notification_preference_user` (`user_id`),
  CONSTRAINT `fk_notification_preference_user` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
...
CREATE TABLE `notification_template` (
  ...
  `code` varchar(100) NOT NULL COMMENT 'Unique template code',
  `category` varchar(50) NOT NULL COMMENT 'Notification category',
  `type` varchar(50) NOT NULL COMMENT 'Notification type',
  `title_template` varchar(120) NOT NULL COMMENT 'Title template',
  `content_template` text NOT NULL COMMENT 'Content template',
  `action_url_template` varchar(500) DEFAULT NULL COMMENT 'Frontend route template',
  `default_priority` int NOT NULL DEFAULT '0' COMMENT 'Default priority',
  ...
  KEY `idx_notification_template_category_type` (`category`,`type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Notification rendering templates';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB00 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='系统权限点表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_activity_log` (
  ...
  KEY `idx_project_activity_commit` (`commit_id`),
  KEY `idx_project_activity_mr` (`merge_request_id`),
  KEY `fk_project_activity_log_branch` (`branch_id`),
  KEY `fk_project_activity_log_check` (`check_run_id`),
  CONSTRAINT `fk_project_activity_log_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_activity_log_check` FOREIGN KEY (`check_run_id`) REFERENCES `project_check_run` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_activity_log_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_activity_log_mr` FOREIGN KEY (`merge_request_id`) REFERENCES `project_merge_request` (`id`) ON DELETE SET NULL,
...
--
-- Table structure for table `project_blob`
--

DROP TABLE IF EXISTS `project_blob`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_blob` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_blob_hash_size` (`sha256`,`size_bytes`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='文件内容对象表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_branch` (
  ...
  `branch_type` varchar(20) NOT NULL DEFAULT 'feature',
  `protected_flag` tinyint(1) NOT NULL DEFAULT '0',
  `allow_direct_commit_flag` tinyint(1) NOT NULL DEFAULT '0',
  `created_by` bigint DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_branch_repo_name` (`repository_id`,`name`),
  KEY `fk_project_branch_created_by` (`created_by`),
  KEY `fk_project_branch_head_commit` (`head_commit_id`),
  CONSTRAINT `fk_project_branch_created_by` FOREIGN KEY (`created_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_branch_head_commit` FOREIGN KEY (`head_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_branch_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目分支表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_check_run`
--

DROP TABLE IF EXISTS `project_check_run`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_check_run` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `repository_id` bigint NOT NULL,
  `commit_id` bigint DEFAULT NULL,
  `merge_request_id` bigint DEFAULT NULL,
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_project_check_run_repository` (`repository_id`),
  KEY `fk_project_check_run_commit` (`commit_id`),
  KEY `fk_project_check_run_mr` (`merge_request_id`),
  CONSTRAINT `fk_project_check_run_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_check_run_mr` FOREIGN KEY (`merge_request_id`) REFERENCES `project_merge_request` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_check_run_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目检查任务表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_commit` (
  ...
  `display_sha` varchar(40) DEFAULT NULL,
  `message` varchar(500) NOT NULL,
  `commit_type` varchar(20) NOT NULL DEFAULT 'normal',
  `snapshot_id` bigint DEFAULT NULL,
  `operator_id` bigint DEFAULT NULL,
  `base_commit_id` bigint DEFAULT NULL,
  `is_merge_commit` tinyint(1) NOT NULL DEFAULT '0',
  `is_revert_commit` tinyint(1) NOT NULL DEFAULT '0',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_commit_repo_branch_no` (`repository_id`,`branch_id`,`commit_no`),
  KEY `fk_project_commit_branch` (`branch_id`),
  KEY `fk_project_commit_operator` (`operator_id`),
  KEY `fk_project_commit_snapshot` (`snapshot_id`),
  KEY `fk_project_commit_base_commit` (`base_commit_id`),
  CONSTRAINT `fk_project_commit_base_commit` FOREIGN KEY (`base_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_commit_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_commit_operator` FOREIGN KEY (`operator_id`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_commit_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_commit_snapshot` FOREIGN KEY (`snapshot_id`) REFERENCES `project_snapshot` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目提交表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_commit_change`
--

DROP TABLE IF EXISTS `project_commit_change`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_commit_change` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `commit_id` bigint NOT NULL,
  `project_file_id` bigint DEFAULT NULL,
  `old_blob_id` bigint DEFAULT NULL,
  `new_blob_id` bigint DEFAULT NULL,
  `old_path` varchar(1000) DEFAULT NULL,
  `new_path` varchar(1000) DEFAULT NULL,
  `change_type` varchar(20) NOT NULL,
  `diff_summary_json` json DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_project_commit_change_commit` (`commit_id`),
  KEY `fk_project_commit_change_file` (`project_file_id`),
  CONSTRAINT `fk_project_commit_change_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_commit_change_file` FOREIGN KEY (`project_file_id`) REFERENCES `project_file` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='提交变更表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_commit_parent`
--

DROP TABLE IF EXISTS `project_commit_parent`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_commit_parent` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `commit_id` bigint NOT NULL,
  `parent_commit_id` bigint NOT NULL,
  `parent_order` int NOT NULL DEFAULT '1',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_commit_parent` (`commit_id`,`parent_commit_id`),
  KEY `fk_project_commit_parent_parent` (`parent_commit_id`),
  CONSTRAINT `fk_project_commit_parent_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_commit_parent_parent` FOREIGN KEY (`parent_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='提交父节点表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_file` (
  ...
  `canonical_path` varchar(1000) DEFAULT NULL,
  `canonical_path_hash` char(64) GENERATED ALWAYS AS ((case when (`canonical_path` is null) then NULL else upper(sha2(`canonical_path`,256)) end)) STORED COMMENT '路径哈希，用于唯一索引',
  `file_key` varchar(1100) DEFAULT NULL,
  ...
  `latest_blob_id` bigint DEFAULT NULL,
  `latest_version_id` bigint DEFAULT NULL,
  `latest_commit_id` bigint DEFAULT NULL,
  `is_latest` tinyint(1) DEFAULT '1' COMMENT '是否为最新版本',
  `deleted_flag` tinyint(1) NOT NULL DEFAULT '0',
  `content_hash` varchar(64) DEFAULT NULL,
  `last_modified_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_file_repo_path_hash` (`project_id`,`canonical_path_hash`,`deleted_flag`),
  KEY `idx_project_upload_time` (`project_id`,`upload_time`),
  KEY `idx_project_file_repository` (`repository_id`),
  KEY `idx_project_file_latest_commit` (`latest_commit_id`),
  KEY `fk_project_file_latest_blob` (`latest_blob_id`),
  CONSTRAINT `fk_project_file_latest_blob` FOREIGN KEY (`latest_blob_id`) REFERENCES `project_blob` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_file_latest_commit` FOREIGN KEY (`latest_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_file_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
CREATE TABLE `project_file_version` (
  ...
  `commit_id` bigint DEFAULT NULL,
  `blob_id` bigint DEFAULT NULL,
  `version_seq` int DEFAULT NULL,
  `content_hash` varchar(64) DEFAULT NULL,
  `path_at_version` varchar(1000) DEFAULT NULL,
  ...
  KEY `idx_project_file_version_commit` (`commit_id`),
  KEY `idx_project_file_version_blob` (`blob_id`),
  KEY `idx_project_file_version_repo_seq` (`repository_id`,`version_seq`),
  KEY `fk_project_file_version_parent` (`parent_version_id`),
  KEY `idx_project_file_version_file_commit_uploaded` (`file_id`,`commit_id`,`uploaded_at`),
  CONSTRAINT `fk_project_file_version_blob` FOREIGN KEY (`blob_id`) REFERENCES `project_blob` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_file_version_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_file_version_parent` FOREIGN KEY (`parent_version_id`) REFERENCES `project_file_version` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_file_version_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
--
-- Table structure for table `project_merge_request`
--

DROP TABLE IF EXISTS `project_merge_request`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_merge_request` (
  ...
  `target_head_commit_id` bigint NOT NULL,
  `title` varchar(255) NOT NULL,
  `description` text,
  `status` varchar(20) NOT NULL DEFAULT 'open',
  `created_by` bigint DEFAULT NULL,
  `merged_by` bigint DEFAULT NULL,
  `merged_at` datetime DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_project_mr_repository` (`repository_id`),
  KEY `fk_project_mr_source_branch` (`source_branch_id`),
  KEY `fk_project_mr_target_branch` (`target_branch_id`),
  KEY `fk_project_mr_created_by` (`created_by`),
  KEY `fk_project_mr_merged_by` (`merged_by`),
  CONSTRAINT `fk_project_mr_created_by` FOREIGN KEY (`created_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_mr_merged_by` FOREIGN KEY (`merged_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_mr_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_mr_source_branch` FOREIGN KEY (`source_branch_id`) REFERENCES `project_branch` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_mr_target_branch` FOREIGN KEY (`target_branch_id`) REFERENCES `project_branch` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目合并请求表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_milestone` (
  ...
  `repository_id` bigint DEFAULT NULL,
  `branch_id` bigint DEFAULT NULL,
  `anchor_commit_id` bigint DEFAULT NULL,
  `from_commit_id` bigint DEFAULT NULL,
  `to_commit_id` bigint DEFAULT NULL,
  ...
  KEY `idx_project_status` (`project_id`,`status`),
  KEY `idx_created_by` (`created_by`),
  KEY `idx_project_milestone_anchor_commit` (`anchor_commit_id`),
  KEY `fk_project_milestone_repository` (`repository_id`),
  KEY `fk_project_milestone_branch` (`branch_id`),
  KEY `fk_project_milestone_from_commit` (`from_commit_id`),
  KEY `fk_project_milestone_to_commit` (`to_commit_id`),
  CONSTRAINT `fk_project_milestone_anchor_commit` FOREIGN KEY (`anchor_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_milestone_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_milestone_from_commit` FOREIGN KEY (`from_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_milestone_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_milestone_to_commit` FOREIGN KEY (`to_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
...
CREATE TABLE `project_release` (
  ...
  `repository_id` bigint DEFAULT NULL,
  `branch_id` bigint DEFAULT NULL,
  `based_commit_id` bigint DEFAULT NULL,
  `based_milestone_id` bigint DEFAULT NULL,
  `package_blob_id` bigint DEFAULT NULL,
  ...
  KEY `idx_project_release_based_commit` (`based_commit_id`),
  KEY `idx_project_release_based_milestone` (`based_milestone_id`),
  KEY `fk_project_release_repository` (`repository_id`),
  KEY `fk_project_release_branch` (`branch_id`),
  KEY `fk_project_release_package_blob` (`package_blob_id`),
  CONSTRAINT `fk_project_release_based_commit` FOREIGN KEY (`based_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_release_based_milestone` FOREIGN KEY (`based_milestone_id`) REFERENCES `project_milestone` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_release_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_release_package_blob` FOREIGN KEY (`package_blob_id`) REFERENCES `project_blob` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_release_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
...
CREATE TABLE `project_repository` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_repository_project` (`project_id`),
  KEY `fk_project_repository_created_by` (`created_by`),
  KEY `fk_project_repository_default_branch` (`default_branch_id`),
  KEY `fk_project_repository_head_commit` (`head_commit_id`),
  KEY `fk_project_repository_current_release` (`current_release_id`),
  CONSTRAINT `fk_project_repository_created_by` FOREIGN KEY (`created_by`) REFERENCES `user_info` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_repository_current_release` FOREIGN KEY (`current_release_id`) REFERENCES `project_release` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_repository_default_branch` FOREIGN KEY (`default_branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_repository_head_commit` FOREIGN KEY (`head_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_repository_project` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目仓库主表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_review` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_project_review_mr` (`merge_request_id`),
  KEY `fk_project_review_reviewer` (`reviewer_id`),
  CONSTRAINT `fk_project_review_mr` FOREIGN KEY (`merge_request_id`) REFERENCES `project_merge_request` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_review_reviewer` FOREIGN KEY (`reviewer_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目评审表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_snapshot`
--
...
CREATE TABLE `project_snapshot` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_project_snapshot_repository` (`repository_id`),
  KEY `fk_project_snapshot_commit` (`commit_id`),
  CONSTRAINT `fk_project_snapshot_commit` FOREIGN KEY (`commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_snapshot_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目快照表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_snapshot_item`
--

DROP TABLE IF EXISTS `project_snapshot_item`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_snapshot_item` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `snapshot_id` bigint NOT NULL,
  `project_file_id` bigint NOT NULL,
  `project_file_version_id` bigint NOT NULL,
  `blob_id` bigint NOT NULL,
  `canonical_path` varchar(1000) NOT NULL,
  `canonical_path_hash` char(64) GENERATED ALWAYS AS (upper(sha2(`canonical_path`,256))) STORED,
  `content_hash` varchar(64) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_snapshot_item_path_hash` (`snapshot_id`,`canonical_path_hash`),
  KEY `fk_project_snapshot_item_file` (`project_file_id`),
  KEY `fk_project_snapshot_item_version` (`project_file_version_id`),
  KEY `fk_project_snapshot_item_blob` (`blob_id`),
  CONSTRAINT `fk_project_snapshot_item_blob` FOREIGN KEY (`blob_id`) REFERENCES `project_blob` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_snapshot_item_file` FOREIGN KEY (`project_file_id`) REFERENCES `project_file` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_snapshot_item_snapshot` FOREIGN KEY (`snapshot_id`) REFERENCES `project_snapshot` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_snapshot_item_version` FOREIGN KEY (`project_file_version_id`) REFERENCES `project_file_version` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目快照明细表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_sprint` (
  ...
  `repository_id` bigint DEFAULT NULL,
  `branch_id` bigint DEFAULT NULL,
  `start_commit_id` bigint DEFAULT NULL,
  `end_commit_id` bigint DEFAULT NULL,
  ...
  KEY `idx_project_status` (`project_id`,`status`),
  KEY `idx_created_by` (`created_by`),
  KEY `idx_project_sprint_branch` (`branch_id`),
  KEY `fk_project_sprint_repository` (`repository_id`),
  KEY `fk_project_sprint_start_commit` (`start_commit_id`),
  KEY `fk_project_sprint_end_commit` (`end_commit_id`),
  CONSTRAINT `fk_project_sprint_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_sprint_end_commit` FOREIGN KEY (`end_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_sprint_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_sprint_start_commit` FOREIGN KEY (`start_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE SET NULL,
...
) ENGINE=InnoDB0 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='任务操作日志表：用于记录任务的状态流转、指派变更、优先级调整等历史';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `project_template_file` (
  ...
  `mime_type` varchar(100) DEFAULT NULL,
  `include_content` tinyint(1) DEFAULT '0',
  ...
  KEY `idx_ptf_template_sort` (`template_id`,`sort_order`,`id`),
  KEY `idx_ptf_item_type` (`item_type`),
...
CREATE TABLE `project_workspace` (
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `active_owner_id` bigint DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_workspace_active_owner_expr` (`repository_id`,`branch_id`,((case when (`status` = _utf8mb4'active') then `owner_id` else NULL end))),
  KEY `fk_project_workspace_repository` (`repository_id`),
  KEY `fk_project_workspace_branch` (`branch_id`),
  KEY `fk_project_workspace_owner` (`owner_id`),
  KEY `fk_project_workspace_base_commit` (`base_commit_id`),
  KEY `idx_project_workspace_lookup` (`repository_id`,`branch_id`,`owner_id`,`status`,`updated_at`),
  CONSTRAINT `fk_project_workspace_base_commit` FOREIGN KEY (`base_commit_id`) REFERENCES `project_commit` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_workspace_branch` FOREIGN KEY (`branch_id`) REFERENCES `project_branch` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_workspace_owner` FOREIGN KEY (`owner_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_project_workspace_repository` FOREIGN KEY (`repository_id`) REFERENCES `project_repository` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目工作区表';
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `project_workspace_item`
--

DROP TABLE IF EXISTS `project_workspace_item`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `project_workspace_item` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `workspace_id` bigint NOT NULL,
  `canonical_path` varchar(1000) NOT NULL,
  `temp_storage_path` varchar(1000) DEFAULT NULL,
  `blob_id` bigint DEFAULT NULL,
  `change_type` varchar(20) NOT NULL,
  `staged_flag` tinyint(1) NOT NULL DEFAULT '1',
  `conflict_flag` tinyint(1) NOT NULL DEFAULT '0',
  `detected_message` varchar(500) DEFAULT NULL,
  `canonical_path_hash` char(64) GENERATED ALWAYS AS (upper(sha2(`canonical_path`,256))) STORED,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_workspace_item_path_hash` (`workspace_id`,`canonical_path_hash`),
  KEY `fk_project_workspace_item_workspace` (`workspace_id`),
  KEY `fk_project_workspace_item_blob` (`blob_id`),
  CONSTRAINT `fk_project_workspace_item_blob` FOREIGN KEY (`blob_id`) REFERENCES `project_blob` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_project_workspace_item_workspace` FOREIGN KEY (`workspace_id`) REFERENCES `project_workspace` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='项目工作区明细表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB00134 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='地区信息表，支持多级行政区域划分';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0511 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='标签信息表，支持树状层级结构，用于对博客和项目进行分类';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0073 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户基本信息表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB0195 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户技能标签关联表，用于展示用户擅长的技术';
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB003 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='内容浏览日志表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2026-04-27 10:35:48
