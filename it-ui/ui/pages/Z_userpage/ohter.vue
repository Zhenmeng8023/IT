        ...
        <div class="profile-card">
          <div class="profile-header">
            <div class="avatar-wrapper">
              ...
              <div v-if="!$route.params.id" class="avatar-edit-overlay" @click="openEditDialog">
          ...
          <el-button v-if="!$route.params.id" type="primary" class="edit-profile-btn" @click="openEditDialog">
...
export default {
  ...
  created() {
    this.getUserInfo();
    // getUserStats 会在 getUserInfo 成功后自动调用
    this.getCityList();
    this.getTagList();
  },
  watch: {
  ...
  methods: {
    ...
    getUserInfo() {
      ...
      if (process.client) {
        ...
        console.log("完整的 route params:", this.$route.params);

        // 如果有 userId 参数（且不是字符串 ':id'），则获取其他用户的公开信息
    ...
    getUserPublicInfo(userId) {
      ...
      // 暂时使用 GetUserById 替代，因为后端的 /api/users/:id/public 接口返回 400 错误
      console.log('准备调用 GetUserById API, userId:', userId);
      GetUserById(userId)
        .then((response) => {
          console.log("获取用户信息成功:", response);
          const userInfo = response.data || response;
          ...
          // 处理头像 URL
          if (userInfo.avatarUrl) {
            // 检查是否是本地绝对路径（包含盘符）或 file:///格式的 URL
            const isLocalPath = /^[A-Za-z]:\\/i.test(userInfo.avatarUrl) || /^file:\/\//i.test(userInfo.avatarUrl);
            if (isLocalPath) {
              // 本地路径或 file:///URL，由于浏览器安全限制，使用默认头像
              this.avatarUrl = '/pic/choubi.jpg';
              console.log('检测到本地路径或 file:///URL，使用默认头像');
            } else {
              // 其他情况，直接使用
              this.avatarUrl = userInfo.avatarUrl;
            }
    ...
    async getUserStats() {
      ...
      try {
        ...
        this.userStats = {
          totalLikes: likesRes.data?.count || likesRes.data?.length || 0,
          totalCollects: collectsRes.data?.count || collectsRes.data?.length || 0,
          followersCount: 0,
          historyCount: 0, // 暂时设为 0，因为后端没有历史记录 API
